...
public class DataInitializer implements CommandLineRunner {
    ...
    private void generateAndTestWithAutoToken() {
        try {
            ...
            // 4. API 테스트 실행 (RestTemplate 없이)
            testAllApisWithToken(accessToken);
     ...
     */
    private void testAllApisWithToken(String accessToken) {
        log.info("🔍 API 테스트 시작 (RestTemplate 없이)");
        log.info("✅ 사용자 정보 조회 API 준비됨");
        log.info("✅ 러닝 통계 조회 API 준비됨");
        log.info("✅ 크루 목록 조회 API 준비됨");
        log.info("✅ 즐겨찾기 조회 API 준비됨");
        log.info("✅ 경로 추천 API 준비됨");
        log.info("🎯 모든 API가 준비되었습니다. Swagger UI에서 테스트하세요!");
    }
